    ...
    <div style={{ minHeight: '100vh', background: 'var(--color-bg)' }}>
      ...
      <section style={{ maxWidth: 720, margin: '0 auto', padding: '48px 20px 32px', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'var(--space-4)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-5)', width: '100%', maxWidth: 360, marginTop: 8 }}>
          {/* For elever — ingen konto, ingen innlogging. */}
          <div style={gruppe}>
            <span style={gruppeEtikett}>For elever</span>
            <div style={{ display: 'flex', gap: 'var(--space-2)' }}>
              <Button variant="primary" onClick={() => navigate(ROUTES.GLOSEMESTER)} style={{ flex: 1 }}>
                Øv uten innlogging
              </Button>
              <Button variant="secondary" onClick={() => navigate(ROUTES.QUIZ)} style={{ flex: 1 }}>
                Ta en prøve
              </Button>
            </div>
          </div>
      ...
      <section style={{ maxWidth: 640, margin: '0 auto', padding: '0 20px 56px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 16 }}>
          <div style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-lg)', padding: 24, boxShadow: 'var(--shadow-card)' }}>
            <h3 style={{ fontWeight: 800, fontSize: 'var(--font-size-lg)', marginBottom: 8 }}>Elever</h3>
            <p style={{ color: 'var(--color-text-muted)', lineHeight: 1.5, margin: 0 }}>
              Øving, prøver og alle kortpakker er alltid gratis — uten innlogging og uten konto.
            </p>
          </div>
          <div style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-lg)', padding: 24, boxShadow: 'var(--shadow-card)' }}>
            <h3 style={{ fontWeight: 800, fontSize: 'var(--font-size-lg)', marginBottom: 8 }}>Lærere</h3>
            <p style={{ color: 'var(--color-text-muted)', lineHeight: 1.5, margin: 0 }}>
              Logg inn med Feide for å lage og dele prøver, ubegrenset og gratis.
            </p>
          </div>
        </div>
      </section>
