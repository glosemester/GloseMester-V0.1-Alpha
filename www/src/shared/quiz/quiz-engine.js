...
export async function startQuiz(prove, containerSelector = '#app') {
    ...
    // Hvis innlogget: bruk Firebase-navn direkte, ellers spør om navn
    const innloggetBruker = auth.currentUser;
    if (innloggetBruker) {
        const brukerNavn = window.GloseMester?.bruker?.displayName
            || innloggetBruker.displayName
            || innloggetBruker.email?.split('@')[0]
            || 'Elev';
        sessionStorage.setItem('elevNavn', brukerNavn);
        sessionStorage.setItem('elevUid', innloggetBruker.uid);
    } else {
        sessionStorage.removeItem('elevUid');
                ...
                <div style="min-height:100vh;display:flex;align-items:center;justify-content:center;padding:24px;background:var(--background,#F5F3FF);">
                    <div style="background:white;border-radius:24px;padding:40px 32px;max-width:400px;width:100%;text-align:center;box-shadow:0 8px 32px rgba(124,58,237,0.10);">
                        ...
                        <button id="elev-navn-btn"
                            style="width:100%;padding:14px;background:#FF6B47;color:white;border:none;border-radius:14px;font-size:17px;font-weight:700;cursor:pointer;">
                            🚀 Start prøven
                        </button>
                    </div>
                </div>`;
    ...
    container.innerHTML = `
        <div class="quiz-wrapper" style="min-height:100vh; padding:20px; max-width:680px; margin:0 auto; display:flex; flex-direction:column; gap:20px;">
            ...
            <div style="height:6px; background:#e5e7eb; border-radius:999px; overflow:hidden;">
                <div style="height:100%; width:${prosent}%; background:linear-gradient(90deg,#FF6B47,#FFB347); border-radius:999px; transition:width 0.4s;"></div>
            </div>
...
function renderTekstfelt(spm) {
    return `
        <form id="quiz-tekst-form" style="display:flex; gap:10px;">
            ...
            <button type="submit" style="padding:16px 24px; background:#FF6B47; color:white; border:none; border-radius:14px; font-size:18px; font-weight:700; cursor:pointer;">
                ✓
            </button>
        </form>
    `;
...
async function visResultat(state, container) {
    ...
    container.innerHTML = `
        <div style="min-height:100vh; padding:40px 20px; max-width:560px; margin:0 auto; display:flex; flex-direction:column; align-items:center; justify-content:center; gap:24px; text-align:center;">
            ...
            <button onclick="window.router?.push(window.GloseMester?.bruker ? '/hjem' : '/')" style="padding:16px 40px; background:#FF6B47; color:white; border:none; border-radius:16px; font-size:17px; font-weight:700; cursor:pointer;">
                🏠 Tilbake
            </button>
        </div>
    `;
...
async function lagreResultat(prove, riktige, totalt, prosent, svar, tidSekunder) {
    ...
    const elevUid  = sessionStorage.getItem('elevUid')  || null;
