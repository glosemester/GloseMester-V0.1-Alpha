...
export class GloseMester extends FagModul {
                ...
                <header class="quiz-header">
                    ...
                    <div class="quiz-info">
                        ...
                        <div class="quiz-stats">
                            ...
                            <span id="trade-token-badge" style="background:#FFE8E3;border:1.5px solid #FFCAB8;border-radius:99px;padding:3px 10px;font-size:12px;font-weight:700;color:#FF6B47;display:none;">🔄 0 trades</span>
                        </div>
                    </div>
    ...
    renderMultipleChoice(correctWord) {
        ...
        // Prefer same-category distractors (uses the k field on niva1 words)
        const sameCategory = correctWord.k
            ? this.currentWords.filter(w => w.k === correctWord.k && w[checkKey] !== correctWord[checkKey])
            : [];
        const otherWords = this.currentWords.filter(w => w[checkKey] !== correctWord[checkKey] && !sameCategory.includes(w));

        const pool = [...this.shuffleArray(sameCategory), ...this.shuffleArray(otherWords)];
        let attempts = 0;
        while (alternatives.length < 4 && attempts < pool.length) {
            const candidate = pool[attempts];
            if (!alternatives.some(alt => alt[checkKey] === candidate[checkKey])) {
    ...
    updateCreditsProgress() {
        ...
        // Skjul baren til brukeren har startet å tjene XP
        if (totalXP === 0) { container.innerHTML = ''; return; }
        ...
        container.innerHTML = `
            <div style="padding: 12px; background: rgba(251, 191, 36, 0.1); border-radius: var(--radius-md, 20px);">
                <div style="display: flex; justify-content: space-between; margin-bottom: 6px; font-size: 11px; font-weight: 600; color: var(--text-dark, #1F2937);">
                    <span>💎 Mot neste diamantbonus</span>
        ...
        return `
            <div style="background:white;border-radius:20px;padding:20px 24px;margin-bottom:20px;box-shadow:0 2px 12px rgba(255,107,71,0.08);border:1.5px solid rgba(255,107,71,0.12);">
                ...
                <div style="background:#f0e9ff;border-radius:99px;height:12px;overflow:hidden;">
                    <div style="background:linear-gradient(90deg,#FF6B47,#FFB347);height:12px;border-radius:99px;width:${pct}%;transition:width 0.4s;"></div>
                </div>
    ...
    updateTradeProgress() {
        ...
        // Vis kun trade-badge når brukeren faktisk har trades tilgjengelig
        if (badge) {
            if (tokens > 0) {
        ...
        if (!container) return;
        // Skjul trade-progress til brukeren har passert første terskel (35 XP)
        ...
        container.innerHTML = `
            <div style="padding:10px 12px;background:rgba(255,107,71,0.07);border-radius:14px;margin-bottom:8px;">
                ...
                ${tokens > 0 ? `<div style="margin-top:5px;font-size:11px;color:#15803d;font-weight:700;text-align:right;">✅ ${tokens} byttemønt klar til bruk</div>` : ''}
            </div>
        `;
    }
    ...
    visOnboarding() {
        ...
        const render = () => {
            const s = steger[steg];
            const dots = steger.map((_, i) => `<span style="width:8px;height:8px;border-radius:50%;background:${i === steg ? '#FF6B47' : '#ddd'};display:inline-block;"></span>`).join('');
            overlay.innerHTML = `
                <div style="background:white;border-radius:28px;padding:36px 32px;max-width:360px;width:100%;text-align:center;box-shadow:0 20px 60px rgba(0,0,0,0.3);">
                    ...
                    <div style="display:flex;gap:10px;justify-content:center;">
                        ...
                        <button id="ob-neste" style="background:#FF6B47;border:none;border-radius:20px;padding:10px 28px;font-size:14px;font-weight:700;color:white;cursor:pointer;">
    ...
    visInnstillinger() {
        ...
        const toggle = (key, label, desc) => `
            <div style="display:flex;align-items:center;justify-content:space-between;padding:14px 0;border-bottom:1px solid #f0f0f0;">
                ...
                <label style="position:relative;display:inline-block;width:44px;height:24px;flex-shrink:0;margin-left:12px;">
                    ...
                    <span style="position:absolute;cursor:pointer;inset:0;background:${s[key] ? '#FF6B47' : '#ccc'};border-radius:24px;transition:0.3s;">
        ...
        overlay.innerHTML = `
            <div style="background:white;border-radius:24px;padding:28px 24px;max-width:360px;width:100%;box-shadow:0 20px 60px rgba(0,0,0,0.25);">
                ...
                <div style="display:flex;gap:10px;margin-top:24px;">
                    ...
                    <button id="inn-lagre" style="flex:1;background:#FF6B47;border:none;border-radius:20px;padding:12px;font-size:14px;font-weight:700;color:white;cursor:pointer;">Lagre</button>
                </div>
            </div>
        `;
        ...
        overlay.querySelectorAll('input[data-key]').forEach(inp => {
            inp.addEventListener('change', () => {
                ...
                if (inp.checked) { span.style.background = '#FF6B47'; knob.style.left = '23px'; }
    ...
    showKortPopup(kort) {
        ...
        popup.innerHTML = `
            <div class="kort-popup playful-card" style="background: white; border-radius: var(--radius-xl, 50px); padding: 40px; max-width: 400px; text-align: center; animation: bounceIn 0.5s;">
                ...
                <h2 style="font-size: 28px; margin-bottom: 15px; color: var(--color-primary, #FF6B47);">Du vant et kort!</h2>
        ...
        container.innerHTML = `
            <div style="padding: 20px;">
                <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 20px; flex-wrap: wrap;">
                    ...
                    <button id="gallery-trade-hub-btn" class="btn" style="padding: 8px 16px; background:#FF6B47; color:white; border:none; border-radius:20px; font-weight:700; cursor:pointer;">Bytt</button>
        ...
        container.innerHTML = `
            <div style="padding: 20px;">
                <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 8px; flex-wrap: wrap;">
                    ...
                    <button id="all-gallery-trade-hub-btn" class="btn" style="padding: 8px 16px; background:#FF6B47; color:white; border:none; border-radius:20px; font-weight:700; cursor:pointer;">Bytt</button>
    ...
    showWrongAnswerPopup(correctAnswer) {
        ...
        popup.innerHTML = `
            <div class="wrong-answer-popup playful-card" style="background: white; border-radius: var(--radius-lg, 30px); padding: 30px; max-width: 350px; text-align: center;">
                ...
                <p style="font-size: 24px; font-weight: 700; color: var(--color-primary, #FF6B47); margin-bottom: 30px;">${correctAnswer}</p>
